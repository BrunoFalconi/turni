...
(function () {
  ...
  function employeeDeduction(annualTaxable) {
    ...
    /*
     * Per ora lasciata a zero.
     * In seguito possiamo collegarla alla detrazione
     * da lavoro dipendente già usata dalla tua app.
     */
    return 0;
  }
  ...
  function calculateProjectedMonthlyIrpef(monthlyTaxable) {
    ...
    const annualGrossTax =
      progressiveTax(annualTaxable);

    const annualDeduction =
      employeeDeduction(annualTaxable);
  ...
  function getPayrollProfile(year, month) {
    ...
    return {
      gross:
        Number(settings.gross) || 0,

      divisor:
        Number(settings.divisor) || 173,

      socialPct:
        Number(settings.socialPct) || 0,
    ...
    settings.employeeDeduction
      ...
      nightPct:
        Number(settings.nightPct) || 50,

      holidayPct:
        Number(settings.holidayPct) || 50,
  ...
  profile.cometaEmployee
    ...
    return {
      ...
      additions: {
        ...
        holidayNight: holidayNightAmount,
        total: additions
      },
      grossTotal,
      socialContributions,
