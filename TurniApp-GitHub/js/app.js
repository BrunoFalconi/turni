const KEY='turni:app';
const LEGACY_KEYS=['turni:v5','turni:v4','turni:v3','turni:v2','turni:v1'];

/* Salvataggio persistente nel browser. I dati restano sul dispositivo anche dopo
   aggiornamenti e ricaricamenti. Le vecchie versioni vengono migrate automaticamente. */
const mem={};
const store={
  mode:(()=>{try{const k='turni:storage-test';localStorage.setItem(k,'1');localStorage.removeItem(k);return 'browser'}catch(e){return 'memoria'}})(),
  async get(k){
...
function saveNow(){
  try{
    if(store.mode==='browser') localStorage.setItem(KEY,JSON.stringify(state));
    else mem[KEY]=JSON.stringify(state);
    return true;
  }catch(e){
...
function save(){
  if(!saveNow()) toast('Salvataggio non riuscito: memoria del browser non disponibile');
}
...
document.getElementById('settings').onclick=()=>{
  ...
  document.getElementById('store-info').innerHTML={
    ...
    browser:'Archivio: <b>salvato sul dispositivo</b><br>I turni restano disponibili dopo la ricarica e anche offline.',
